...
public class Shift2Adapter extends  RecyclerView.Adapter<Shift2Adapter.MyViewHolder> implements Filterable {
    List<AttendanceListResponse2> attendanceListResponses;
    Context context;

    // SEARCH VIEW FILTER **************************************************************************
    private List<AttendanceListResponse2> al_searchlist;

    IRestService service;

    public Shift2Adapter(ShiftTwoActivity shiftTwoActivity, List<AttendanceListResponse2> attendanceListResponses) {
        this.context=shiftTwoActivity;
        this.attendanceListResponses=attendanceListResponses;

        // SEARCH VIEW FILTER **************************************************************************
        this.al_searchlist = new ArrayList<AttendanceListResponse2>();
        al_searchlist.addAll(attendanceListResponses);
    }

    public void setNameList(Context context,final List<AttendanceListResponse2> mArrayList){
...
// CODE FOR SEARCH VIEW FILTER *********************************************************************
        final DiffUtil.DiffResult result = DiffUtil.calculateDiff(new DiffUtil.Callback() {
            ...
            public int getOldListSize() {
                return Shift2Adapter.this.al_searchlist.size();
            }
            ...
            public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                return Shift2Adapter.this.al_searchlist.get(oldItemPosition).getName() == mArrayList.get(newItemPosition).getName();
            }

            @Override
            public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {

                AttendanceListResponse2 newMovie = Shift2Adapter.this.al_searchlist.get(oldItemPosition);

                AttendanceListResponse2 oldMovie = mArrayList.get(newItemPosition);
    ...
    @Override
    public void onBindViewHolder(@NonNull final Shift2Adapter.MyViewHolder holder, int position) {


        final AttendanceListResponse2 attendanceListResponse = attendanceListResponses.get(position);
                ...
                service.getAttend(s1,s2).enqueue(new Callback<AttendResponse>() {
                    ...
                    public void onResponse(Call<AttendResponse> call, Response<AttendResponse> response) {
                        ...
                        if(status.equals("success")){
                            ...
                            // Intent i = new Intent(context,ShiftTwoActivity.class);
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    List<AttendanceListResponse2> filteredList = new ArrayList<>();

                    for (AttendanceListResponse2 androidVersion : al_searchlist) {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                attendanceListResponses = (List<AttendanceListResponse2>) filterResults.values;
