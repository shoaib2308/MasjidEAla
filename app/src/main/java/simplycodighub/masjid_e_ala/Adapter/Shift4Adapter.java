...
import simplycodighub.masjid_e_ala.View.ShiftFourActivity;
import simplycodighub.masjid_e_ala.View.ShiftOneActivity;
import simplycodighub.masjid_e_ala.View.ShiftTwoActivity;

public class Shift4Adapter extends  RecyclerView.Adapter<Shift4Adapter.MyViewHolder> implements Filterable {
    List<AttendanceListResponse4> attendanceListResponses = new ArrayList<>();
    Context context;

    IRestService service;
    // SEARCH VIEW FILTER **************************************************************************
    private List<AttendanceListResponse4> al_searchlist;

    public Shift4Adapter(ShiftFourActivity shiftFourActivity, List<AttendanceListResponse4> attendanceListResponses) {
        this.context=shiftFourActivity;
        this.attendanceListResponses=attendanceListResponses;

        // SEARCH VIEW FILTER **************************************************************************
        this.al_searchlist = new ArrayList<AttendanceListResponse4>();
        al_searchlist.addAll(attendanceListResponses);
    }

    public void setNameList(Context context,final List<AttendanceListResponse4> mArrayList){
...
// CODE FOR SEARCH VIEW FILTER *********************************************************************
        final DiffUtil.DiffResult result = DiffUtil.calculateDiff(new DiffUtil.Callback() {
            ...
            public int getOldListSize() {
                return Shift4Adapter.this.al_searchlist.size();
            }
            ...
            public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                return Shift4Adapter.this.al_searchlist.get(oldItemPosition).getName() == mArrayList.get(newItemPosition).getName();
            }

            @Override
            public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {

                AttendanceListResponse4 newMovie = Shift4Adapter.this.al_searchlist.get(oldItemPosition);

                AttendanceListResponse4 oldMovie = mArrayList.get(newItemPosition);
    ...
    @Override
    public void onBindViewHolder(@NonNull final Shift4Adapter.MyViewHolder holder, int position) {


        final AttendanceListResponse4 attendanceListResponse = attendanceListResponses.get(position);
            ...
            service.getAttend(s1,s2).enqueue(new Callback<AttendResponse>() {
                ...
                public void onResponse(Call<AttendResponse> call, Response<AttendResponse> response) {
                    ...
                    if(status.equals("success")){
                       ...
                       // Intent i = new Intent(context,ShiftFourActivity.class);
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    List<AttendanceListResponse4> filteredList = new ArrayList<>();

                    for (AttendanceListResponse4 androidVersion : al_searchlist) {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                attendanceListResponses = (List<AttendanceListResponse4>) filterResults.values;
