...
import simplycodighub.masjid_e_ala.View.ShiftZeroActivity;

public class Shift0Adapter extends  RecyclerView.Adapter<Shift0Adapter.MyViewHolder> implements Filterable {
    List<AttendanceListResponse0> attendanceListResponses;
    Context context;


    // SEARCH VIEW FILTER **************************************************************************
    private List<AttendanceListResponse0> al_searchlist;

    IRestService service;

    public Shift0Adapter(ShiftZeroActivity shiftZeroActivity, List<AttendanceListResponse0> attendanceListResponses) {
        this.context=shiftZeroActivity;
        this.attendanceListResponses=attendanceListResponses;

        // SEARCH VIEW FILTER **************************************************************************
        this.al_searchlist = new ArrayList<AttendanceListResponse0>();
        al_searchlist.addAll(attendanceListResponses);
    }

    public void setNameList(Context context,final List<AttendanceListResponse0> mArrayList){
...
// CODE FOR SEARCH VIEW FILTER *********************************************************************
        final DiffUtil.DiffResult result = DiffUtil.calculateDiff(new DiffUtil.Callback() {
            ...
            public int getOldListSize() {
                return Shift0Adapter.this.al_searchlist.size();
            }
            ...
            public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                return Shift0Adapter.this.al_searchlist.get(oldItemPosition).getName() == mArrayList.get(newItemPosition).getName();
            }

            @Override
            public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {

                AttendanceListResponse0 newMovie = Shift0Adapter.this.al_searchlist.get(oldItemPosition);

                AttendanceListResponse0 oldMovie = mArrayList.get(newItemPosition);
    ...
    @Override
    public void onBindViewHolder(@NonNull Shift0Adapter.MyViewHolder holder, int position) {


        final AttendanceListResponse0 attendanceListResponse = attendanceListResponses.get(position);
                ...
                service.getAttend(s1,s2).enqueue(new Callback<AttendResponse>() {
                    ...
                    public void onResponse(Call<AttendResponse> call, Response<AttendResponse> response) {
                        ...
                        if(status.equals("success")){
                            ...
                            Intent i = new Intent(context,ShiftZeroActivity.class);
                            context.startActivity(i);
                            ((Activity)context).finish();  // BEST WAY FINISH FROm ADAPTER
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    List<AttendanceListResponse0> filteredList = new ArrayList<>();

                    for (AttendanceListResponse0 androidVersion : al_searchlist) {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                attendanceListResponses = (List<AttendanceListResponse0>) filterResults.values;
